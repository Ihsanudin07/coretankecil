...
@section('content')
        ...
        <div class="col-xl-3 col-md-6 mb-4">
            <div class="card border-left-info shadow h-100 py-2">
                <div class="card-body">
                    <div class="row no-gutters align-items-center">
                        <div class="col mr-2">
                            <div class="text-xs font-weight-bold text-info text-uppercase mb-1">Tag
                            </div>
                            <div class="row no-gutters align-items-center">
                                <div class="col-auto">
            ...
            @if ($post->count() >= 1)
                ...
                <table class="table table-hover table-bordered">
                    ...
                    @foreach ($post as $row)
                        ...
                        <td><img src="/upload/post/{{ $row->sampul }}" alt="" width="150px" height="100px"></td>
                        <td>{{ $row->judul }}</td>
                        <td>{{ $row->kategori->nama }}</td>
                        <td>
                            @foreach ($row->tag as $post_tag)
                            <span class="badge badge-warning">{{ $post_tag->nama }}</span>
                            @endforeach
                        </td>
                        </tr>
                    @endforeach
    ...
    <div class="card border-bottom-warning mt-4">
        <div class="card-header ">
            <h6 class="m-0 font-weight-bold text-warning">BannerHari Ini</h6>
        </div>
        <div class="card-body">
            @if ($banner->count() >= 1)
                {{-- Tabel --}}
                <table class="table table-hover table-bordered">
                    ...
                    @foreach ($banner as $row)
                        <tr>
                        <th scope="row">{{ $loop->iteration }}</th>
                        <td><img src="/upload/banner/{{ $row->sampul }}"  width="150px" height="100px" alt=""></td>
                        <td>{{ $row->judul }}</td>
                        <td>{{ $row->slug }}</td>
                        </tr>
                    @endforeach
